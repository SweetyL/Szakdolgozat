...
function adminConfirmAdd() {
    ...
    if(!check || !check2 || !check5 || !check6 || length > 200 || !(dateNow.getTime() <= dateInput.getTime())){
        Swal.fire({
            icon: 'error',
            title: 'Hiba!',
            text: 'Hibás adatok!'
    ...
    if(!check){
        ...
        Swal.fire({
            ...
            cancelButtonText: 'Vissza'
              ...
              if (result.isConfirmed) {
                document.adminDel.submit();
              }
        })
    }
}

function adminConfirmModify(){
    let check = $("#modJob").val();
    let check2 = $("#modTrip").val();
    let check3 = $("#modTruck").val();
    let check4 = $("#modReward").val();
