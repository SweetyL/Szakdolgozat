    ...
    isset($_POST['username']) and isset($_POST['password'])) {
        ...
        if ($conn->query($sql)) {
            if (!($result->num_rows > 0)) {
                $sql2 = "INSERT INTO `drivers`(`firstname`, `lastname`, `townID`, `street`, `houseNumber`, `email`, `phoneNumber`, `username`, `password`) VALUES ('".htmlspecialchars(mysqli_real_escape_string($conn,$_POST['firstname']))."','".htmlspecialchars(mysqli_real_escape_string($conn,$_POST['lastname']))."','".htmlspecialchars(mysqli_real_escape_string($conn,$_POST['town']))."','".htmlspecialchars(mysqli_real_escape_string($conn,$_POST['street']))."','".htmlspecialchars(mysqli_real_escape_string($conn,$_POST['houseNumber']))."','".htmlspecialchars(mysqli_real_escape_string($conn,$_POST['email']))."','".htmlspecialchars(mysqli_real_escape_string($conn,$_POST['phoneNumber']))."','".htmlspecialchars(mysqli_real_escape_string($conn,$_POST['username']))."',MD5('".htmlspecialchars(mysqli_real_escape_string($conn,$_POST['password']))."'));";
